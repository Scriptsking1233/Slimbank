...
(function () {
  ...
  /* ---------- balance edited in Supabase wins over the local one ---------- */
  function currentUser() {
    var map = readUsers();
    var id = api.loginId;
    if (!id) { try { id = localStorage.getItem(KS); } catch (e) {} }
    return id && map[id] ? { map: map, id: id, u: map[id] } : null;
  }
  function saveUser(cur) {
    cur.map[cur.id] = cur.u;
    writeUsers(cur.map);
    if (window.SLIM_HOOKS && window.SLIM_HOOKS.reload) {
      try { window.SLIM_HOOKS.reload(cur.u); } catch (e) {}
    }
  }
  function applyOverride(bal) {
    var cur = currentUser();
    if (!cur || bal == null) return;
    var n = Number(bal);
    if (!isFinite(n) || Number(cur.u.balance) === n) return;
    cur.u.balance = n;
    saveUser(cur);
    badge("\u0421\u0435\u0440\u0432\u0435\u0440: \u0431\u0430\u043b\u0430\u043d\u0441 \u0438\u0437\u043c\u0435\u043d\u0451\u043d \u0430\u0434\u043c\u0438\u043d\u043e\u043c", "ok");
    log("balance override", n);
  }
  api.applyOverride = applyOverride;

  /* ---------- names owned by somebody else are removed ---------- */
  var warned = {};
  function dropNames(list) {
    var cur = currentUser();
    if (!cur) return;
    var bad = {}, i, gone = [];
    for (i = 0; i < list.length; i++) bad[String(list[i]).toLowerCase()] = 1;
    if (Object.prototype.toString.call(cur.u.ownNames) === "[object Array]") {
      var keep = [];
      for (i = 0; i < cur.u.ownNames.length; i++) {
        var nm = String(cur.u.ownNames[i] || "").replace(/^@/, "");
        if (bad[nm.toLowerCase()]) gone.push(nm); else keep.push(cur.u.ownNames[i]);
      }
      cur.u.ownNames = keep;
    }
    ...
    for (i = 0; i < gone.length; i++) {
      if (!warned[gone[i].toLowerCase()]) { warned[gone[i].toLowerCase()] = 1; fresh.push("@" + gone[i]); }
    }
    if (fresh.length) {
      badge("\u0421\u0435\u0440\u0432\u0435\u0440: \u0438\u043c\u044f \u0437\u0430\u043d\u044f\u0442\u043e \u0434\u0440\u0443\u0433\u0438\u043c", "warn");
      try { alert("\u042d\u0442\u0438 \u044e\u0437\u0435\u0440\u043d\u0435\u0439\u043c\u044b \u0443\u0436\u0435 \u0437\u0430\u043d\u044f\u0442\u044b \u0434\u0440\u0443\u0433\u0438\u043c\u0438 \u0438\u0433\u0440\u043e\u043a\u0430\u043c\u0438 \u0438 \u0443\u0431\u0440\u0430\u043d\u044b \u0438\u0437 \u043f\u0440\u043e\u0444\u0438\u043b\u044f: " + fresh.join(", ")); } catch (e) {}
    }
  }
  api.dropNames = dropNames;

  api.claimName = function (name) {
    if (!getToken()) return Promise.resolve({ ok: false, error: "no_session" });
    return rpc("sb_claim_name", { p_token: getToken(), p_name: String(name || "") });
  };
  api.myNames = function () {
    if (!getToken()) return Promise.resolve([]);
    return rpc("sb_my_names", { p_token: getToken() }).then(function (r) {
      return r && r.names ? r.names : [];
    });
  };
  api.nameOwner = function (name) {
    return rpc("sb_name_owner", { p_name: String(name || "") });
  };
...
(function () {
  ...
  function wire() {
    ...
    /* ---- auction: reserve a username on the server before buying ---- */
    if (!document.body.dataset.slimAucWired) {
      document.body.dataset.slimAucWired = "1";
      document.addEventListener("click", function (ev) {
        var el = ev.target;
        while (el && el !== document.body && !el.getAttribute) el = el.parentElement;
        var lot = null;
        while (el && el !== document.body) {
          if (el.getAttribute && el.getAttribute("data-auclot")) { lot = el; break; }
          el = el.parentElement;
        }
        if (!lot) return;
        var raw = lot.getAttribute("data-auclot") || "";
        var txt = (lot.textContent || "") + " " + raw;
        var m = txt.match(/@([A-Za-z0-9_]{3,20})/);
        if (!m) { setTimeout(function () { api.syncNow && api.syncNow(true); }, 900); return; }
        var name = m[1];
        if (lot.dataset.slimOk === name) return;
        if (!getToken()) return;
        ev.preventDefault();
        ev.stopPropagation();
        badge("\u0421\u0435\u0440\u0432\u0435\u0440: \u043f\u0440\u043e\u0432\u0435\u0440\u044f\u0435\u043c \u0438\u043c\u044f...", "warn");
        api.claimName(name).then(function (r) {
          if (r && r.ok) {
            lot.dataset.slimOk = name;
            lot.click();
            setTimeout(function () { api.syncNow && api.syncNow(true); }, 700);
          } else {
            badge("\u0421\u0435\u0440\u0432\u0435\u0440: \u0438\u043c\u044f \u0443\u0436\u0435 \u0437\u0430\u043d\u044f\u0442\u043e", "warn");
            try { alert("@" + name + " \u0443\u0436\u0435 \u043a\u0443\u043f\u0438\u043b \u0434\u0440\u0443\u0433\u043e\u0439 \u0438\u0433\u0440\u043e\u043a"); } catch (e) {}
          }
        });
      }, true);
    }
